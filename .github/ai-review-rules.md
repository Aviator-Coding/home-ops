...
- The Talos `allowedVersions` pin in `.renovate/overrides.json5` (excludes exactly v1.13.3, not a
  version ceiling) gates a live, unattended upgrade: `tuppr`'s `TalosUpgrade` CR is Flux-applied with
