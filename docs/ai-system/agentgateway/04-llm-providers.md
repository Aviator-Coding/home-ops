...
| `embedding-local` | in-cluster `embedding-gpu.ai:8080`, OpenAI-compat | default `/v1/embeddings` | B70 llama.cpp SYCL, `Qwen3-Embedding-0.6B` (moved off CPU TEI 2026-09-15; model swapped 2026-09-13 from `all-MiniLM-L6-v2`); not `vllm-embed` (still scaled to 0, no route targets it) |
...
Redeployed 2026-08-26 as a governance layer (`../litellm/README.md`), not as part of this gateway's routing. Unified `/v1` is still AgentGateway itself. Do not create a LiteLLM `AgentgatewayBackend`, and do not tell gateway clients to use LiteLLM's Service or its internal HTTPRoute (`litellm.${SECRET_DOMAIN}` on `envoy-internal`) - those are a separate path (direct local model, optional D3 `auto` complexity router, Phase 5 `chat-ha` fallback alias, per-consumer virtual keys; see `../litellm/auto-router.md` and `../litellm/fallbacks.md`) with none of this gateway's routing/fallback/auth. Public exposure of LiteLLM remains forbidden.
