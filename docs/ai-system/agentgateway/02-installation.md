...
```
kubernetes/apps/ai/kustomization.yaml          # includes ./agentgateway/ks.yaml
kubernetes/apps/ai/agentgateway/
  ks.yaml                                      # Flux Kustomizations: agentgateway + agentgateway-crds
  crds/
...
Live values are in [`app/helmrelease.yaml`](../../../kubernetes/apps/ai/agentgateway/app/helmrelease.yaml). Notes already in that file:
...
[`crds/helmrelease.yaml`](../../../kubernetes/apps/ai/agentgateway/crds/helmrelease.yaml) installs `agentgateway-crds` and keeps CRDs on Helm uninstall so deleting the chart cannot cascade-delete every backend/policy.

## What the old "Home-Ops install" pages got wrong

`02` and `11` used to paste kgateway v2.1.2 manifests under `kubernetes/apps/ai-system/`, list kagent/kmcp/kgateway as apps, and disagree with each other on `crds: CreateReplace` vs `Skip`. The live app HelmRelease uses `CreateReplace`. The live namespace kustomization is [`kubernetes/apps/ai/kustomization.yaml`](../../../kubernetes/apps/ai/kustomization.yaml) - no kagent, kmcp, or kgateway.
