...
(exactly 30 days, every calendar date). `"1mo"` is a *calendar* month clamped
to the target month's length (28-31 days depending on the date the proxy
computes it), so it is not a fixed 30-day span and is not a safe stand-in for
Barman's fixed `30d` window - which is why it was not used. Pinned by
`scripts/ci/litellm-request-logs-test.py`
