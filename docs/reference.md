...
| [`kubernetes/components/kopiur/Readme.md`](../kubernetes/components/kopiur/Readme.md) | kopiur backup component (authoritative migration status, schedules, wait:true split, per-namespace r2 hours, credential projection, mover-identity, SecurityContextCompatible semantics, rollback). |
...
| [`ai-system/litellm/claude-code-subscription.md`](ai-system/litellm/claude-code-subscription.md) | Claude Code Max/Pro pass-through (Sonnet + Opus): OAuth token forwarding, full $0 pricing incl. cache fields, client `ANTHROPIC_DEFAULT_*_MODEL` contract, no cluster Anthropic key. Use when wiring the `claude` CLI through the proxy. |
...
Fleet PVC backup remains VolSync on every protected claim (live claim list and `ReplicationSource` count: [`kubernetes/components/volsync/Readme.md`](../kubernetes/components/volsync/Readme.md)). kopiur runs **alongside** VolSync on onboarded claims; live count, deferred claims, and restore proofs are owned by [`components/kopiur/Readme.md`](../kubernetes/components/kopiur/Readme.md). Nothing has been retired - that is Stage 5. Operator/repos: [`kopiur/README.md`](../kubernetes/apps/base/system/kopiur/README.md).
