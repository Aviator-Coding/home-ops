...
### [2026-06-14] OSD device-path drift hardening (Rook #17224): runbook + audit  (PR #984; corrected #985)

| Field | Value |
|-------|-------|
| **Change** | Docs/tooling: runbook [`docs/ceph/osd-device-path-recovery.md`](./ceph/osd-device-path-recovery.md), `task rook:check-osd-device-paths` (+ script), CLAUDE.md reboot guardrail, this entry. (#984 also added `cephClusterSpec.osdMaxUpdatesInParallel: 1` — **reverted in #985 as inert**; see ⚠️ note below.) |
...
| **Risk** | None — docs + a read-only task. Underlying Rook limitation unchanged (no config fixes it). Live-verified: relocate fallback works under HEALTH_OK on all 3 nodes; 4/6 OSD deployments (osd.3/4/5/6) are stale-but-running (harmless until restarted). |
| **Rollback** | `git revert` (docs/task only). |
| **Verify** | `task rook:check-osd-device-paths` → per-OSD drift + exits non-zero unless HEALTH_OK. Recovery = patch `ROOK_BLOCK_PATH` to the correct kernel name + force-delete the stuck pod (runbook Case A). |

⚠️ **`osdMaxUpdatesInParallel: 1` was added in #984 then reverted in #985 — it is a no-op here.** The field is valid in Rook v1.19+/v1.20.0 upstream, but the **cluster's CephCluster CRD is behind the v1.20.0 operator**: the field is absent from the live CRD (`kubectl explain cephcluster.spec.osdMaxUpdatesInParallel` → "field does not exist"), so the API server **silently prunes** the value. On v1.20.0 the operator already gates OSD takedowns on **`ok-to-stop`** (the core safety; a `size=3`/3-host cluster serializes per host anyway). **FOLLOW-UP (deferred): the Rook CRDs did not fully reconcile to v1.20.0 despite a "successful" Helm upgrade** (the ~944 KB `resources.yaml` — known Flux/Helm + large-CRD friction). Investigate + refresh the CRDs, then pin `osdMaxUpdatesInParallel: 1`.
