...
| `kopiur-epoch-tuning-test.py` | kopiur `ceph` epoch tuning (2026-09-03): parsed manifest pins `parameters.epoch.minDuration` inside the derived `(3.0h, 5.85h]` band - the band that advances the epoch at every quick run and never at the full run - self-checks the band itself clears the 1000-blob threshold with >=40% headroom at the measured 32.5 blobs/hour, and pins `r2` as deliberately untuned WITH its explanatory comment, and refuses any `spec.health.indexBlobWarnThreshold` on either repository. Catches "correcting" 5h to the 6h the alert message suggests, and silencing the warning instead of fixing it |
