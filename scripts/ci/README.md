...
| `vllm-memory-alert-test.py` | ai/vllm memory-approach PrometheusRule (2026-09-14): asserts the rendered rule's structural contract (`predict_linear` window/horizon, `for:` durations, thresholds, vllm-embed exclusion, severity labels), then drives `promtool test rules` with synthetic series to prove the growth-aware `VLLMMemoryApproachingLimit` warning stays silent on a noisy-but-non-climbing 48h band, fires only once a sustained climb's projection has held for the full `for: 1h` window, that the static `VLLMMemoryCriticalLimit` backstop still fires/resolves on its 0.85 threshold, and that the sibling vllm-embed controller can never trigger either alert |
