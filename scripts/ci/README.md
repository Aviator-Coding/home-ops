...
| `fstrim-pv-selection-test.py` | system/fstrim PVC discard path (2026-09-06): runs the shipped awk selector on kubelet-namespace-shaped mountinfo (globalmount wins, major:minor de-dupe, RO/non-dev dropped, EPHEMERAL xfs kept), proves pre-fix `grep -v kubelet` yields zero PVCs, exercises the discovery pipeline split fail-closed under `set -eu`, pins Monday/`Forbid`/`backoffLimit: 0`, nfs mgr `enabled: false`, and deliberate absence of `mountOptions: [discard]` |
