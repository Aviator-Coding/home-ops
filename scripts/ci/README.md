...
| `samba-shared-xml-test.py` | LAN SMB shared-xml share (2026-09-04): standalone 100Gi RWX `ceph-filesystem-rwx` claim, no backup engine, dockur/samba LoadBalancer `10.50.0.55:445` + ownership/setgid/postStart contract, hermes `/opt/xml` RW app-only mount + `dependsOn ai-pvc` (render-only; live Mac drill is out of band) |
