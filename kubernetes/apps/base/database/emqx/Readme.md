...
- **Exporter API key bootstrap is manual.** The EMQX operator hardcodes `EMQX_API_KEY__BOOTSTRAP_FILE=/opt/emqx/data/bootstrap_api_key` (env, which wins over HOCON) and auto-mounts a Secret it generates and owns at that path. We can't override the env var or layer additional keys via the CR. To set up the exporter:
  1. Open `https://mqtt.${SECRET_DOMAIN}` and log in with `EMQX_DEFAULT_USERNAME` / `EMQX_DEFAULT_PASSWORD`.
  2. System -> API Keys -> "+ Create new API Key", name it `exporter`, role `Administrator`. Copy the api_key + api_secret (shown once).
  3. Update 1Password fields `EMQX_EXPORTER_API_KEY` and `EMQX_EXPORTER_API_SECRET` with the values from step 2.
  4. `kubectl annotate externalsecret -n database emqx-exporter force-sync=$(date +%s) --overwrite` then `kubectl rollout restart deployment -n database emqx-exporter`.
...
- **QoS can still land Burstable even with matching requests/limits on the EMQX container.** k8tz's injected init already carries `initContainerResources` requests==limits (`system-controller/k8tz`), so the inject itself no longer demotes Guaranteed pods; any remaining Burstable class comes from another container in the pod (operator sidecars, etc.), not from a resource-less k8tz init.
- **Dashboards live under the Database Grafana folder** (alongside CNPG and Dragonfly). The 7 EMQX dashboards (`emqx-overview`, `-authentication`, `-authorization`, `-client-events`, `-messages`, `-rule-engine-count`, `-rule-engine-rate`) are loaded directly from raw URLs in `apps/base/monitoring/grafana/app/helmrelease.yaml` and require the exporter to populate. Without the exporter (or with an exporter that can't authenticate), they all read empty.
