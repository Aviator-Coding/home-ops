...
- Resources: `requests` and `limits` both at 250m CPU + 640Mi memory. CR-level matching requests/limits are necessary for Guaranteed QoS but not always sufficient — see Operational notes.
...
- **QoS can still land Burstable even when the CR sets matching `requests` and `limits`.** k8tz's injected init already carries `initContainerResources` requests==limits (`system-controller/k8tz`), so the inject itself no longer demotes Guaranteed pods; any remaining Burstable class comes from another container in the pod (operator sidecars, etc.), not from a resource-less k8tz init. Not a Dragonfly-side fix.
