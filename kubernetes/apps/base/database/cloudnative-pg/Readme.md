...
| `offsite-mirror/` | Hourly CronJob that copies the `postgres-17` Barman archive (LAN MinIO -> Cloudflare R2), giving it a second, off-site backup copy. CNPG can only express one live barman destination, so this mirrors the archive rather than adding a second `ScheduledBackup`. Details, credential and known follow-ups: `offsite-mirror/README.md`. |
...
- **Off-site mirror** of that archive to R2 runs hourly, independently of the above; see `offsite-mirror/README.md`.
