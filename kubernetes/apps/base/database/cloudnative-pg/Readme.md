...
| `cluster-17/` | The `postgres-17` Cluster CR (Postgres 17 + pgvecto.rs), its `ScheduledBackup` (`@daily`), `LoadBalancer` Service, Gatus probe, PrometheusRule (11 alerts), and Barman config (serverName `postgres17-v5`, MinIO bucket `s3://home-ops-postgres-cluster/`). |
...
- `cluster-17/prometheusrule.yaml` defines the 11 alerts that operate on `cnpg_*` metrics (including primary-only backup-age and WAL-archive staleness, each paired with a `*TimestampAbsent` companion for when the primary drops or zeroes the timestamp); rules are cluster-wide so they cover any future CNPG cluster too.
