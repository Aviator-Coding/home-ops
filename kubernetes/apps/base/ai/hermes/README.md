...
**Every restart of this pod is an unclean exit, by construction.** `container-boot.log` records
`prior_exit` on each container start: the last **10 consecutive boots** (2026-09-04 → 2026-09-15)
all read `unclean`, and all 6 recorded `SIGTERM`s produced one. That is arithmetic, not bad luck:
